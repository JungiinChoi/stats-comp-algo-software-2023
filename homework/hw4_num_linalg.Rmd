...
numerical_errors <- function(cond_num = 1e6) {
  ...
  for (Method in c("lu", "chol", "qr")) {
    error_df <- data.frame(method = Method, 
                           error = rel_error_list[[Method]]) %>%
      pivot_longer(!method, names_to = "metric", values_to = "rel_error")
    rel_error_df <- rbind(rel_error_df, error_df)
  }
  
  rel_error_df %>%
    ggplot(aes(x = rel_error)) + 
    geom_density(aes(group = method, fill = method), alpha = 0.3) + 
