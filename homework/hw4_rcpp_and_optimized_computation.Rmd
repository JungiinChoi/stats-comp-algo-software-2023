...
for (i in 1:1000){
  ...
  v <- rep(1, vec_size)
  w <- rep(1, vec_size)
  inner_c <- c(inner_c, inner_prod(v, w))
}
...
For Rcpp implementation, 971 out of 1000 returned 1022 while the other 29 out of 1000 were way far from 1022 (e.g. -8.7e+245, 8.2e+193, etc). 

In R base implementation, they also disagree because the majority of the dot product results were NaN. 

In Rcpp, index starts from 0 while R uses 1-starting index system. This makes difference when they store vectors from initial index for each language and thus results in inconsistency of dot product.  
...
The first column always gives 1024 for all rows. But, the second row gives different values every time I run this code chunk.  
...
**Answer:**
R session is aborted when I run the first code chunk and second code chunk works well by returning expected results. This is because `x` is longer than `y`. This causes the function to write data outside the memory reserved for the "y" vector, which can cause errors in the program. The second code chunk works properly because the function is only applied to the first 1000 elements of "y". However, this is not what one would expect from a vector-scalar product, which should only be applied to vectors of the same length. To make the `axpy_c()` function safe, we should add a check at the beginning of the function to make sure that "x" and "y" are the same length.
...
```{r}
row_col_AB = row_col_dot_matmat(A, B)
base_AB = A %*% B
are_all_close(Rcpp_AB, base_AB)
```
...
```{r}
col_oriented_AB = col_oriented_matmat(A, B)
are_all_close(col_oriented_AB, row_col_AB)
...
Column-oriented matrix-vector multiplication is much faster than the standard way of multiplying them by rows and columns. The reason for this is that when we use column-oriented calculation, the required data is more likely to be in cache compare to standard multiplication, reducing the time it takes to access the data. 
...
**Answer:**
Although the column-oriented matrix-vector multiplication is significantly faster than the standard row-column method, its performance is closer to the built-in BLAS library. 
